...
API_URL = "https://api.semanticscholar.org/graph/v1/paper/DOI:{doi}?fields=title,abstract,year,authors"

# Function to get abstract from Semantic Scholar
def get_paper_abstract_semanticscholar(doi):
    url = API_URL.format(doi=doi)
    headers = {
#        "x-api-key": "your_semantic_scholar_api_key"  # Get your API key from https://www.semanticscholar.org/product/api
    }
...
with open(csv_file, mode='w', newline='', encoding='utf-8') as file:
    ...
    for doi in extracted_dois:
        ...
        abstract = get_paper_abstract_semanticscholar(doi)
        if abstract is None: 
           abstract = "..."
        writer.writerow([doi, title, abstract])
